    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage your hired freelancers and active projects.</p>
        </div>
        <Button asChild>
          <Link to="/client/jobs/post">Post a New Job</Link>
        </Button>
      </div>
      ...
      {contracts.length === 0 ? (
        <Card className="flex flex-col items-center justify-center py-16 text-center border-dashed">
          ...
          <CardDescription className="max-w-sm mb-6">
            You haven't hired anyone yet. Review bids on your open jobs to start a contract.
          </CardDescription>
          <Button asChild variant="outline">
            <Link to="/client/jobs/my">View My Jobs</Link>
          </Button>
        </Card>
      ) : (
                ...
                <div className="p-6">
                  <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                    <div className="space-y-1 flex-1">
                      {contract.job ? (
                        <Link to={`/client/contracts/${contract.id}`} className="hover:text-indigo-600 transition-colors">
                      ...
                      <div className="flex items-center gap-3 text-sm text-muted-foreground">
                        ...
                        {contract.freelancer && (
                          <span className="flex items-center text-gray-700 font-medium ml-2">
                            Freelancer: {contract.freelancer.full_name}
                          </span>
                        )}
                      </div>
                    </div>
